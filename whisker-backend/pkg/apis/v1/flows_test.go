...
import (
	...
	"github.com/projectcalico/calico/whisker-backend/pkg/testutil"
)
	...
	tt := []struct {
		...
		},
		{
			...
			request: mustCreateGetRequest(t, "GET", "/api/v1/flows", map[string][]string{
				"filters": {testutil.MustMarshal(t, v1.Filters{SourceName: "foobar"})}}),
			expected: &v1.ListFlowsParams{Filters: v1.Filters{SourceName: "foobar"}},
		},
	}
